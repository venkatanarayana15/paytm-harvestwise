...
| 17 | **The whole live demo is unrecoverable** (projector/laptop swap, network dead, provider outage) | play the recorded run fullscreen and say: *"this is a pre-recorded run of the exact flow"* | `backup_demo/demo_backup.mp4` — real recorded Full Auto run (1360×850 H.264, ~23 s) + `stage_*.png` proof stills + `demo_transcript.txt` step log. Regenerate: `node qa-runs/pw/record_backup.js` (⚠ drives a real dispatch — reset after) | ✅ recorded 2026-09-18 |
