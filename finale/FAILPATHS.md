...
| 17 | **The whole live demo is unrecoverable** (projector/laptop swap, network dead, provider outage) | play the recorded run fullscreen and say: *"this is a pre-recorded run of the exact flow"* | **PRIMARY:** `backup_demo/copilot_narrated.mp4` — narrated copilot Q&A run (32 s H.264+AAC): Q&A → Tamil order → adjust → approve → dispatched ₹456. **Alt (voice pipeline):** `backup_demo/demo_backup_narrated.mp4` (34 s) / silent `demo_backup.mp4`. Proof stills + transcripts included. Regenerate: `node qa-runs/pw/record_copilot.js` + `python make_copilot_narrated.py` (⚠ drives a real dispatch — reset after) | ✅ recorded 2026-09-19 |
